...
class Crisscott_Tools_ProductEdit extends GtkTable {
    ...
    private function _layout()
    {
        ...
        $style = $save->style->copy();
        // Change the normal state.
        // Set the background color to dark blue.
        $color = new GdkColor();
        $blue = $color->parse('#0A0A6A');
        $style->bg[Gtk::STATE_NORMAL] = $blue;

        // Make the prelight color white.
        $style->bg[Gtk::STATE_PRELIGHT] = $style->white;
        $save->set_style($style);

        // The label inside the button must be changed too.
        $style = new GtkStyle();
        // Change the normal and prelight states.
        $style->fg[Gtk::STATE_NORMAL]   = $style->white;
        $style->fg[Gtk::STATE_PRELIGHT] = $blue;

        // Root through the button's children and grandchildren.
        foreach ($save->get_child()->get_children() as $child) {
            foreach($child->get_children() as $c) {
                // Set the style.
                $c->set_style($style);
            }
        }
    ...
    public function resetProduct()
    {
        ...
        if ($this->imageContainer->get_child()) {
    ...
    public function saveProduct()
    {
		...
		if (Crisscott_Inventory::$transmitting) {
			...
			$dialog = new GtkMessageDialog(null, $flags, Gtk::MESSAGE_WARNING, 
										   Gtk::BUTTONS_CLOSE, $message);

			// Close the dialog when the user clicks the button.
			$dialog->connect_simple('response', array($dialog, 'destroy'));
        ...
        $this->product->imagePath    = $this->imagePathEntry->get_text();
        ...
        try {
            ...
            // Let the main window know that something has changed.
            require_once 'Crisscott/MainWindow.php';
            Crisscott_MainWindow::$sent = false;
